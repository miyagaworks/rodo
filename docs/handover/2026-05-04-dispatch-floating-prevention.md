...
- **§8.7 書類作成画面のブラウザバック禁止（PWA 運用対応）**: 2026-05-05 ユーザー実機検証中に発見。書類作成 3 画面（RecordClient / ReportOnsiteClient / ReportTransportClient）でブラウザバックを押すと前画面（DispatchClient）に戻れてしまい、隊員が前回のアシスタンス情報を引きずる事故リスクあり。popstate ガード新設 + 専用モーダル通知（「ホームに戻るには画面上のホームボタンを使ってください」）で対応。ホームボタン経由は既存挙動維持。**Phase 8 完了後に独立タスクで着手**。詳細は計画書 §8.7
...
**2026-05-05 実機検証結果: 全シナリオ PASS（ユーザー報告）**

ユーザー miyagawakiyomi による実機検証完了。O-1〜O-5（a/b/c）すべて期待どおりの挙動。詳細端末別記入は省略（簡略報告のため）。Phase 6 完了条件達成。
...
| O-4（2 次搬送で同じ挙動） | PASS | |
| O-5 a（帰社後 isDraft=false で戻る → モーダル） | PASS | 新基準 a |
| O-5 b（モーダル中キャンセル → ホーム） | PASS | 新基準 b |
| O-5 c（出動記録ボタン → 書類画面 → 戻る → 下書きバナー復帰） | PASS | 新基準 c |

**派生発見事項**: 書類作成画面でブラウザバックを押すと前画面（DispatchClient）に戻れてしまう問題を本検証中に発見。§8.7 派生課題として正式起票済み（PWA 運用対応・Phase 8 完了後の独立タスクで実装予定）。Phase 6 PASS 判定には含めない。
