...
- **称賛・同調は禁止**。端的に。
- 壁打ちが長引くと苛立つ → **選択肢を出したら即座に進める**。
- 技術判断は委任される（「判断できない」「OK」「どれでも良い」と言われたら super-agent が即決）。
- 「鈍臭い」と感じさせない迅速さが重要。
- スクリーンショットによる確認を重視。
- 「スタイリッシュかどうか」「業務 UX の妥当性」はユーザー専権。super-agent が判定するのは技術的観点（スコープ遵守・テストグリーン・必須スクショ・既存反省点の再発有無）に限る。
...
3. **壁打ちを長引かせない**。完了報告と「コミットして良いか」の確認は 1 回で済ませる。複数往復は無駄。
