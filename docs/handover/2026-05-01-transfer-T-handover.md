...
## ⚠️ 2026-05-01 セッション末追記（最重要・先に読む）

本ノート §E と §H の手順1〜9（タスクF+G統合 / オフラインバナー修正 / 動作確認 / コミット / push）は **本セッションで実施完了**。
コミット6本（`b20ab13` / `2e6e250` / `8efbb21` / `6ed168d` / `6c166a6` / `b90ef9b`）push 済み（origin 反映済）。
バックフィル SQL（§J）と追加課題は未実施。

### 残課題（次セッションで対応）

| 優先 | 課題 | 詳細 |
|---|---|---|
| 🔴 | handleProceed の res.ok チェック追加 | `RecordClient.tsx` L378-399、サイレント故障の温床 |
| 🔴 | B-28 の不明点確認 | `docs/smoke-test-checklist.md`、ユーザー指定 |
| 🟡 | バックフィル SQL 実行 | §J 参照、対象3件 |
| 🟢 | offlineFetch 呼び出し箇所の res.ok チェック網羅調査 | サイレント故障の他箇所洗い出し |
| 🟢 | lint 既存負債解消（62 errors / 46 warnings） | baseline 固定 + 新規厳格 |

### 本セッションで追加された再発防止策（必読）

- プロジェクト `AGENTS.md` に「業務仕様の真偽判定」「修正前チェックリスト」「修正完了報告のサイレント故障チェック」追加（コミット `6c166a6`）
- Super Agent 定義（`~/.claude/agents/super-agent.md`）に「決め打ち提案の規律」追加 — 根拠なし決め打ち禁止、外部状態（navigator.onLine 等）はブラウザ/OS/SW を疑ってから内部実装を疑う
- これらは次セッションで **必ず読むこと**

### 本セッションで遭遇した重要な落とし穴

1. **navigator.onLine の誤判定**: Mac/Chrome で WiFi 切替等により false に固着することがある。Chrome 完全再起動で解消。コード側でも `lib/offline-fetch.ts` から navigator.onLine 事前判定を削除済（実 fetch 結果ベースに統一、`8efbb21`）
2. **Service Worker のフォールバック 503**: `/api/*` の SW networkOnly 戦略により、ネット失敗時に 503 JSON を返すため、`offlineFetch` がこれを「実ネット断」と誤認していた → `X-SW-Offline: 1` ヘッダで識別する設計に変更済（`8efbb21`）。今後 SW 関連の修正時は `public/sw.js` のキャッシュ戦略を必ず確認

### 次セッションの最初のアクション

1. 本ノート全体を読む（特に §C 業務仕様 と §D 警告）
2. **`AGENTS.md` の「プロジェクト固有ルール」セクション** を読む（本セッションで追加）
3. ユーザーから「B-28 の不明点」を確認 → 修正方針設計
4. 並行で残課題🔴2件（handleProceed res.ok / B-28）を片付ける
5. 余裕があればバックフィル SQL（§J）実行をユーザーに提案
