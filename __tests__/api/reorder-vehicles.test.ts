...
import { POST } from '@/app/api/settings/vehicles/reorder/route'
...
function makeRequest(body: unknown): Request {
  return new Request('http://localhost/api/settings/vehicles/reorder', {
    method: 'POST',
...
describe('POST /api/settings/vehicles/reorder', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('401: 未認証', async () => {
    ...
    const res = await POST(makeRequest({ orderedIds: ['v1'] }))
  ...
  it('400: orderedIds 重複', async () => {
    ...
    const res = await POST(makeRequest({ orderedIds: ['v1', 'v1'] }))
  ...
  it('409: 件数不一致', async () => {
    mockedAuth.mockResolvedValueOnce(adminSession)
    mockedFindMany.mockResolvedValueOnce([{ id: 'v1' }, { id: 'v2' }, { id: 'v3' }])

    const res = await POST(makeRequest({ orderedIds: ['v1', 'v2'] }))
  ...
  it('409: 別テナントの id を含む', async () => {
    mockedAuth.mockResolvedValueOnce(adminSession)
    mockedFindMany.mockResolvedValueOnce([{ id: 'v1' }, { id: 'v2' }])

    const res = await POST(
      makeRequest({ orderedIds: ['v1', 'v2', 'other-tenant-vehicle'] })
    )
  ...
  it('200: 正常系で orderedIds 順に sortOrder が更新される', async () => {
    ...
    const res = await POST(
      makeRequest({ orderedIds: ['v3', 'v1', 'v2'] })
    )
  ...
  it('findMany は tenantId でスコープされ、isActive で絞り込まない（停止中車両も対象）', async () => {
    mockedAuth.mockResolvedValueOnce(adminSession)
    mockedFindMany.mockResolvedValueOnce([{ id: 'v1' }])

    await POST(makeRequest({ orderedIds: ['v1'] }))
